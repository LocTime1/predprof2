...
def admin_func():
    ...
    if request.form.get('choise_client_btn'):
        ...
        print([name, desc, filename])
        if request.form.get('save_new_prize') == 'aaa':
            add_prize([filename, name, desc])
        else:
            update_prize([int(request.form.get('save_new_prize')), filename, name, desc])
    elif request.form.get('delete_prize'):
        delete_prize(int(request.form.get('delete_prize')))
    session['id_field'] = 0
    return render_template('admin.html', sl=get_all_fields(), sl_with_prizes=get_prizes())
...
def create_prize():
    if current_user.is_admin == None:
        return redirect(url_for('client.client_func'))
    return render_template('create_prize.html', prize=[], idd='aaa', flag=False)
...
def create_prize_post():
    ...
    return render_template('create_prize.html', prize=get_prizes()[num_prize], idd=num_prize, flag=True)
